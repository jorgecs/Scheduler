...
class Policy:
    def __init__(self, policy, max_qubits, time_limit_seconds, executeCircuit):
        self.queues = {'ibm': [], 'aws': []}
        self.timers = {'ibm': ResettableTimer(time_limit_seconds, lambda: policy(self.queues['ibm'], max_qubits, 'ibm', executeCircuit)),
                       'aws': ResettableTimer(time_limit_seconds, lambda: policy(self.queues['aws'], max_qubits, 'aws', executeCircuit))}
...
class SchedulerPolicies:
    def __init__(self, app):
        ...
        self.max_qubits = 29
        self.services = {'time': Policy(self.send, self.max_qubits, self.time_limit_seconds, self.executeCircuit),
                        'shots': Policy(self.send_shots, self.max_qubits, self.time_limit_seconds*6, self.executeCircuit),
                        'depth': Policy(self.send_depth, self.max_qubits, self.time_limit_seconds*6, self.executeCircuit),
                        'shots_depth': Policy(self.send_shots_depth, self.max_qubits, self.time_limit_seconds*6, self.executeCircuit),
                        'shots_optimized': Policy(self.send_shots_optimized, self.max_qubits, self.time_limit_seconds*6, self.executeCircuit)}
    ...
    def executeCircuit(self,data,qb,shots,provider,urls): #Data is the composed circuit to execute, qb is the number of qubits per circuit, shots is the number of shots per circut, provider is the provider of the circuit, urls is the array with data of each circuit (url, num_qubits, shots, user, circuit_name)
        circuit = 'def circ():\n'
        f = json.loads(data)
        ...
        try:
            if provider == 'ibm':
                ...
                counts = runAWS(self.machine_aws,loc['circuit'],max(shots),'') #Ejecutar el circuito y obtener el resultado
    ...
    def most_repetitive(self, array): #Check the most repetitive element in an array and if there are more than one, return the smallest
        count_dict = {}
    ...
    def create_circuit(self,urls,code,qb,provider): #TODO add there the returning queue and in the other methods, check if the queue is not empty after the execution of thid method, so it adds the circuits back
        ...
        if provider == 'ibm':
            ...
            code.insert(0,"from qiskit import execute, QuantumRegister, ClassicalRegister, QuantumCircuit, Aer")
    ...
    def send_shots_optimized(self,queue, max_qubits, provider, executeCircuit):
        if len(queue) != 0:
            ...
            data = {"code":code}
            Thread(target=executeCircuit, args=(json.dumps(data),qb,shotsUsr,provider,urls)).start()
            #executeCircuit(json.dumps(data),qb,shotsUsr,provider,urls)
            self.services['shots_optimized'].timers[provider].reset()

    def send_shots_depth(self,queue, max_qubits, provider, executeCircuit):
        # Send the URLs to the server
        if len(queue) != 0:
            qb = []
            sumQb = 0
    ...
    def send_depth(self,queue, max_qubits, provider, executeCircuit):
        # Send the URLs to the server
        if len(queue) != 0:
            print('Sent')
            qb = []
    ...
    def send_shots(self,queue, max_qubits, provider, executeCircuit):
    # Send the URLs to the server
        if len(queue) != 0:
            print('Sent')
            qb = []
            ...
            self.create_circuit(urls,code,qb,provider)
            data = {"code":code}
            Thread(target=executeCircuit, args=(json.dumps(data),qb,shotsUsr,provider,urls)).start() #Parece que sin esto no se resetea el timer cuando termina de componer
            #executeCircuit(json.dumps(data),qb,shotsUsr,provider,urls)
            self.services['shots'].timers[provider].reset()

    def send(self,queue, max_qubits, provider, executeCircuit):
        if len(queue) != 0:
            print('Sent')
            urls = []
